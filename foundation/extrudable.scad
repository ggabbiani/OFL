...
module __test__() {
  ...
  extrudable(verbs,CHAR);
}

function fl_xtr_Character(c) = 
assert(is_string(c) && len(c)==1)
let(

) [];

module extrudable(verbs,type,debug=false,axes=false) {
  assert(verbs!=undef);
  assert(is_string(type)&&len(type)==1);
  ...
  module do_add() {
    ...
    resize(size)
      linear_extrude(size.z)
        text(type,valign="center",halign="center");
