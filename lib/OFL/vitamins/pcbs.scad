        ...
        do_layout("components")
          if ($comp_director==+Z||$comp_director==-Z)
            if ($comp_engine==FL_PHDR_NS)
              translate(+Z(EPS)) fl_pinHeader([FL_DRILL],$comp_type,cut_thick=pcb_t+EPSx2,octant=$comp_octant,direction=$comp_direction);
...
function fl_pcb_Frame(
  ...
  if (scr_specs) fl_screw_specs(value=scr_specs),
