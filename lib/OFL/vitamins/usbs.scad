...
) fl_Object([[-l/2,-(iw1+2*t)/2,0],[+l/2,+(iw1+2*t)/2,h]],
    engine  = "USB/uA",
    others  = [
      fl_cutout(value=[+X]),
      fl_USB_flange(value=true),
  ...
  module usb_A_sm(verb) {
    ...
    module do_add() {
      translate(+Z($this_size.z/2)) {
        fl_color("silver")
          fl_linear_extrude(length=$this_size.x,direction=[-FL_X,0])
            difference() {
              fl_square(size=[$this_size.z,$this_size.y],corners=Rext);
              fl_square(size=[$this_size.z-2*Tshield,$this_size.y-2*Tshield],corners=Rint);
            }
        fl_color(tongue)
          tongue();
        fl_color(fl_grey(30))
          translate(-fl_X($this_size.x-Tshield))
