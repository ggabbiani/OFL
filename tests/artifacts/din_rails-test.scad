...
if (single) {
  fl_DIN_rail(verbs,single(LENGTH,PUNCH),cut_direction=co_direction,cut_drift=drift,octant=octant,direction=direction);
} else {
  all = [for(constructor=FL_DIN_RAIL_INVENTORY) constructor(LENGTH,PUNCH)];
  fl_layout(axis=+X,gap=3,types=all,$FL_LAYOUT="ON")
    fl_DIN_rail(verbs,all[$i],cut_direction=co_direction,cut_drift=drift,octant=octant,direction=direction);
}
