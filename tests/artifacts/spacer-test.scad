...
SCREW  = "4";                     // [none,2,3,4,5]
KNUT   = "double spiral thread";  // [none,linear thread,double spiral thread]
// no fillet if zero
FILLET  = 1;  // [0:0.1:5]
...
screw = SCREW!="none" ? find_screw(hs_cap,atoi(SCREW)) : undef;
knut  = KNUT!="none"  ?
          screw ? fl_knut_search(screw=screw,thick=H,tag=KNUT) : echo("***WARN***: knut ignored since no screw provided") undef
        : undef;
